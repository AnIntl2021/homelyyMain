...
class CatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: type == 0
            ? AllApi().getRestaurantbyCat(title, latlng!.latitude.toString(),
                latlng!.longitude.toString())
            : AllApi().getLifestylebyCat(title, latlng!.latitude.toString(),
                latlng!.longitude.toString()),
        builder: (context, snapshot) {
          ...
          var restomodel = snapshot.requireData as List;
                    ...
                    child: Column(
                      children: [
                        CircleAvatar(
                            ...
                            child: ClipRRect(
                              child: Image.network(
                                  'https://thehomelyy.com/images/category/${image}'),
                            )),
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                                type == 0
                                    ? "${restomodel.length.toString()} Providers"
                                    : "${restomodel.length.toString()} Shops",
...
class _CatListState extends State<CatList> {
  ...
  Widget build(BuildContext context) {
    ...
    var indexofOther =
        widget.catList!.indexWhere((element) => element.name == 'Others');
    ...
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          var image =
              "https://thehomelyy.com/images/category/${widget.catList![index].image}";
          // var collection = product[index]["collection"];
          // var recipe = product[index]["recipe"];
          var img = title != "Jeans"
              ? "https://firebasestorage.googleapis.com/v0/b/food-app-b497c.appspot.com/o/indiawebp.webp?alt=media&token=6e49d6e9-3461-4142-8e35-deeb36d0f886"
          ...
          return CatCard(
              ...
              type: widget.type,
              uid: widget.uid,
              count: widget.catList!.length.toString(),
              latlng: widget.latlng,
              image: widget.catList![index].image ??
                  widget.catList![indexofOther].image
              // price: "\â‚¹ $price",
              );
        });
  }
